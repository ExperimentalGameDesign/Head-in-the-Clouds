...
/*	public GameController game;
	public GameObject selectScreen, redBall, blueBall, greenBall;
	public bool splashing = true;

	private GameObject temp, redB, blueB, greenB;
	...
	void Start () {
		temp = GameObject.Instantiate (selectScreen, new Vector3 (0.0f, 0.0f, 0.0f), Quaternion.identity) as GameObject;
		temp.transform.localScale = new Vector3 (10.0f, 19.0f, 0.0f);
		redB = GameObject.Instantiate (redBall, new Vector3 (-10.0f, 0.0f, 0.0f), Quaternion.identity) as GameObject;
	...
	void Update () {
		if (Input.GetMouseButtonDown(0) && splashing == false) {
			StartFade();
